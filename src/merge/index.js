...
class ImageProcessing {
    constructor (runtime) {
        ...
        const detectModuleLoad = () => {
            if (!this.detectorManager.isLoaded) return this.detectorManager.loadDetector()
        ...
        // Set a delay before starting the loading process
        // setTimeout(() => {
        //     classifierModuleLoad().then(() => setTimeout(detectModuleLoad, 5000));  // 5000ms = 5s delay
        // }, 5000);
    }
    ...
    isFaceDetected(args) {
        ...
        return args.USER_NAME === 'any' ? faces.length > 0 : faces.some(face => face.id === args.USER_NAME);
    }
    ...
    _targetFacePartsPosition (id, faceParts) {
        ...
        face.keypoints.forEach((keypoint, i) => {
            switch (faceParts) {
                ...
                case 'rightEyebrow':
                case 'lips':
                    if (keypoint.name === faceParts) {
                        partPoint = this._getPartPointFace(keypoint, partPoint);
                    }
                    break;
                case 'nose':
    ...
    _getPartPointFace(keypoint, partPoint) {
        if (!partPoint) {
    ...
    isHandposeDetected(args) {
        ...
        return handposes
            .some(result => result.handedness === args.HANDEDNESS || (args.HANDEDNESS === 'Both' && handposes.length === 2));
    }
    ...
    detectedHandposeSize(args) {
        ...
        const [wrist, thumb, middle, pinky] = handpose.filter((_, index) => [0, 4, 12, 20].includes(index));
        const width = Math.hypot(pinky.x - thumb.x, pinky.y - thumb.y);
        const height = Math.hypot(middle.x - wrist.x, middle.y - wrist.y);
        
        return (width + height) / 2;
    }

    _targetHandposePartsPosition(handedness, handposeParts) {
        const handpose = this._targetHandpose(handedness);
        const ret = { x: 0, y: 0 };
        if (!handpose) return ret;

        const isPalm = handposeParts === 'palm';
        const targetKeypoints = isPalm ? handpose
            .filter((_, index) => [0, 9].includes(index)) : handpose.filter(keypoint => keypoint.name.startsWith(handposeParts));
        console.log('targetKeypoints', targetKeypoints);
        const partPoint = this._getPartPointHand(targetKeypoints);
    ...
    isPoseDetected(args) {
        ...
        if (!targetKeypoint) return false;
    ...
    _targetPose(poseSide, poseParts) {
        const pose = this.detectorManager.detectedResult(DetectionType.POSE)[0];
        if (!pose) return false;
        const targetKeypoint = pose.keypoints
