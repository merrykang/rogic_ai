...
class ImageProcessing {
    ...
    // 수정: face-landmarks-detection 의 좌표로 얼굴 크기 계산해주는 함수
    detectedFaceSize (args) {
        const face = this._targetFace(args.USER_NAME);
        if (!face) return 0;

        const jaw1Index = 234;  // faceOval 중 가장 왼쪽 바깥에 있는 인덱스
        const jaw2Index = 454;  // faceOval 중 가장 오른쪽 바깥에 있는 인덱스
        const jaw1 = face.keypoints[jaw1Index];
        const jaw2 = face.keypoints[jaw2Index];
    ...
    // 수정 : face-landmarks-detection 의 좌표 출력해주는 함수
    ...
    // 추가 : face-landmarks-detection 의 partPoint 계산해주는 함수
