...
class ImageProcessing {
        ...
        face.keypoints.forEach((keypoint, i) => {
            switch (type) {
                ...
                case 'rightEye':
                    ...
                    break;
                case 'leftEyeBrow': 
                    if (keypoint.name === 'leftEyebrow') {
                        partPoint = this._getPartPoint(keypoint, partPoint);
                    }
                    break;
                case 'rightEyeBrow': 
                    if (keypoint.name === 'rightEyebrow') {
                        partPoint = this._getPartPoint(keypoint, partPoint);
                    }
                    break;
                case 'mouth': 
                    if (keypoint.name === 'lips') {
                        partPoint = this._getPartPoint(keypoint, partPoint);
                    }
                    break;
                case 'nose':
    ...
    // 추가 : partPoint 계산해주는 함수
    _getPartPoint(landmark, partPoint) {
        if (!partPoint) {
            partPoint = { x: 0, y: 0, count: 0 };
        }
        partPoint.x += landmark.x;
        partPoint.y += landmark.y;
        partPoint.count++;
        return partPoint;
    }
