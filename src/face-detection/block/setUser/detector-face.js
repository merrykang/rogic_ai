        ...
        return new Promise((resolve, reject) => {   
            this._detector.estimateFaces(imageData).then(result => {
                ...
                this._result.forEach((res, i) => {
                    ...
                    // faceMatcher의 _labeledDescriptors 배열에서 _label 값을 가져와 this._result의 id에 할당
                    if (faceMatcher._labeledDescriptors && faceMatcher._labeledDescriptors.length > i) {
                        res.id = faceMatcher._labeledDescriptors[i]._label;
                    }
                })
