...
class HandPoseCamera extends Detector{
  constructor() {
    super();
  ...
  drawCtx(imageData) {
    ...
    this._context.drawImage(this._canvas, 0, 0, this._canvas.width, this._canvas.height);
  }
  ...
  drawResult(hand) {
    ...
    // drawKeypoints() : 키포인트를 그림; 
    console.log(hand.keypoints, hand.handedness);
  ...
  drawKeypoints(keypoints, handedness) {
      const keypointsArray = keypoints;
      this._context.fillStyle = handedness === 'Left' ? 'Red' : 'Blue';
