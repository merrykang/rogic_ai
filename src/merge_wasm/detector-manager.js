...
setWasmPaths({
    'tfjs-backend-wasm.wasm': '/static/tensorflow-models/tfjs_wasm/tfjs-backend-wasm.wasm',
    'tfjs-backend-wasm-simd.wasm': '/static/tensorflow-models/tfjs_wasm/tfjs-backend-wasm-simd.wasm',
    'tfjs-backend-wasm-threaded-simd.wasm': '/static/tensorflow-models/tfjs_wasm/tfjs-backend-wasm-threaded-simd.wasm'
});
...
class DetectorManager {
    ...
    /**
     * 추가: face-api 의 prepare() 함수 대체하기 위한 함수 
     *  isLoaded(), loadDetector()
     */
    get isLoaded () {
        return Object.values(DetectionType).every((type) => this._isExistDetector(type));  // 특정 감지 켜기 전에는 무조건 false 반환
    }    

    loadDetector() {
        return new Promise((resolve, reject) => {
            this.isLoaded ? reject(new Error('Detectors are already loaded.')) : resolve();
        });
    }

    get faceDetector () { // FaceDetector 의 static 메서드인 userList 때문에 필요
        return FaceDetector;
    }
    ...
    _createDetector(type) {
        switch (type) {
            ...
            case DetectionType.HANDPOSE:  // 추가 : hand-pose-detection
    ...
    _enableDetectedBox(type, ghost = 50) {
        if (this._isExistDetector(type)) {
            ...
            if (dt.skinId === -1 && dt.drawable === -1) {
                dt.skinId = this._renderer.createBitmapSkin(
                    new ImageData(...Detector.DIMENSIONS),
                    1
                );
                dt.drawable = this._renderer.createDrawable(
                    StageLayering.IMAGE_PROCESSING_DETECTED_BOX_LAYER
                );
    ...
    _disableDetectedBox(type) {
        if (this._isExistDetector(type)) {
            ...
            if (dt.skinId !== -1 || dt.drawable !== -1) {
                this._renderer.destroyDrawable(
                    dt.drawable,
                    StageLayering.IMAGE_PROCESSING_DETECTED_BOX_LAYER
                );
    ...
    setPreviewGhost(ghost) {
        Object.entries(DetectionType).forEach(([key, type]) => {
            if (
                this._isExistDetector(type) &&
                this._detectors[type].drawable !== -1
            ) {
                this._detectors[type].ghost = ghost;
                this._renderer.updateDrawableEffect(
                    this._detectors[type].drawable,
                    "ghost",
                    ghost
                );
            }
    ...
    // 추가: handpose, face, pose 인 경우에만 wasm 백엔드 사용
    _setBackend(type) {
        const backendType = ['handpose', 'face', 'pose'].includes(type) ? 'wasm' : 'webgpu';
        return tf.setBackend(backendType);
    }

    _detect(type, imageData) {
        if (!this._isExistDetector(type) || !imageData) return;
        const dt = this._detectors[type];
        if (!dt.enable || dt.detecting) return;
    
        dt.detecting = true;
        const hasResult = dt.result ? true : false;
        console.log('backend: ', tf.getBackend());
    
        this._setBackend(type)
            .then(() => dt.detector.detect(imageData))
            .then(result => {
                dt.result = result;
                if (dt.drawBox) {
